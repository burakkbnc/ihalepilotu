...
export default async function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  if (!isSuperAdmin && profile.companyId) {
    ...
    if (!companySnap.exists || (companySnap.data() as any)?.status === 'disabled') redirect('/login?error=company_inactive');
  }

  const sidebarRole = ((profile.role === 'admin_team' || (profile.role === 'super_admin' && profile.adminRole)) ? 'admin_team' : profile.role || 'member') as UserRole;
  ...
  return (
    <div className="min-h-screen bg-[#F5F7FB] text-slate-950">
      ...
      <div className="relative flex min-h-screen">
        <Sidebar role={sidebarRole} displayName={profile.displayName} email={profile.email} isSuperAdmin={isSuperAdmin} adminRole={profile.adminRole} />
